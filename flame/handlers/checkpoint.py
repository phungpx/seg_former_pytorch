...
class BestSaver(ignite.handlers.ModelCheckpoint, Module):
    ...
    def __init__(
        ...
        super(BestSaver, self).__init__(
            ...
            if mode == "max"
            else -e.state.metrics[score_name],
            score_name=score_name,
            n_saved=n_saved,
            atomic=atomic,
            require_empty=require_empty,
            create_dir=create_dir,
            global_step_transform=lambda engine, event: self.frame[
                "engine"
            ].engine.state.epoch,
        )
    ...
    def init(self):
        ...
        self.frame[self.evaluator_name].engine.add_event_handler(
            Events.EPOCH_COMPLETED, self, {"model": self.frame["model"]}
        )
    ...
    def load_state_dict(self, state_dict):
        ...
        self._saved = [
            ignite.handlers.Checkpoint.Item(priority, filename)
...
class BackupSaver(ignite.handlers.ModelCheckpoint, Module):
    ...
    class Checkpoint(object):
        def __init__(self, modules, frame):
            ...
            assert all(
                map(lambda x: x in frame, modules)
            ), f"The frame does not have all {modules}"
        ...
        def state_dict(self):
            checkpoint = {
                module: self.frame[module].state_dict() for module in self.modules
            }
    ...
    def __init__(
        ...
        dirname = os.path.join(dirname, datetime.now().strftime("%y%m%d%H%M"))
        super(BackupSaver, self).__init__(
            dirname,
            "backup",
            n_saved=n_saved,
            atomic=atomic,
    ...
    def init(self):
        ...
        self.frame["engine"].engine.add_event_handler(
            Events.EPOCH_COMPLETED(every=self.save_interval), self._correct_checkpoint
        )
    ...
    def _correct_checkpoint(self, engine):
        ...
        checkpoint[self.module_name]["saved"] = [
            (priority, filename)
            for priority, filename in self.frame[self.module_name]._saved
        ]
...
class CheckpointLoader(Module):
    ...
    def init(self):
        assert "engine" in self.frame, "The frame does not have engine."
        self.frame["engine"].engine.add_event_handler(
            Events.STARTED, self._load_checkpoint, self.mode
        )
    ...
    def _load_checkpoint(self, engine, mode):
        if mode in ["resume"]:
            ...
            self.frame["engine"].engine.state.iteration = checkpoint.pop(
                "last_iteration"
            )
...
class History(Module):
    ...
    def load_state_dict(self, state_dict):
        ...
        self.metric_values = defaultdict(
            lambda: defaultdict(list), state_dict["metric_values"]
        )
