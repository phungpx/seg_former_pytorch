...
class RegionPredictor(Module):
    ...
    def update(self, output):
        ...
        preds = preds.to(torch.uint8).cpu().numpy()
        ...
        for pred, image_name, image_size in zip(preds, image_names, image_sizes):
            image_path = "{}/{}{}".format(
                self.output_dir, Path(image_name).stem, self.output_img_ext
            )
            mask_path = "{}/{}_mask{}".format(
                self.output_dir, Path(image_name).stem, self.output_mask_ext
            )

            image = cv2.imread(image_name)
            mask = [
                cv2.resize(
                    self._rm_small_components(pred[i], class_ratio),
                    dsize=image_size,
            ...
            for (color, _, _), m in zip(self.classes.values(), mask):
                ...
                image[m.astype(np.bool)] = (
                    0.7 * image[m.astype(np.bool)].astype(np.float32)
                    + 0.3 * np.array(color, dtype=np.float32)
                ).astype(np.uint8)

            class_contours = [
                cv2.findContours(m, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
                for m in mask[-1:]
            ]
