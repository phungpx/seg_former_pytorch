...
class EKYCDataset(Dataset):
    def __init__(
        ...
        image_extents: List[str] = ["**/*.*g", "**/*.JPG"],
        label_extents: List[str] = ["**/*.json"],
        ...
        for image_extent in image_extents:
            for image_dir in image_dirs:
                image_paths.extend(Path(image_dir).glob(f'**/{mode}/{image_extent}'))
        label_paths = []
        for label_extent in label_extents:
            for image_dir in image_dirs:
                label_paths.extend(Path(image_dir).glob(f'**/{mode}/{label_extent}'))
        image_paths = natsorted(image_paths, key=lambda x: x.stem)
        label_paths = natsorted(label_paths, key=lambda x: x.stem)
        self.data_pairs = [
            (image_path, mask_path)
            for image_path, mask_path in zip(image_paths, label_paths)
            if image_path.stem == mask_path.stem
        ]
        print(f"{mode} - {len(self.data_pairs)}")
    ...
    def __getitem__(self, idx):
        ...
        mask = cv2.imread(str(image_path), cv2.IMREAD_GRAYSCALE) * 0
        with open(mask_path) as f:
            mask_infos = json.load(f)
            
        for mask_info in mask_infos['shapes']:
