...
if __name__ == '__main__':
    ...
    model = MiT(model_name=version).to(device)
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    x = torch.zeros(size=(1, 3, 224, 224), device=device)
    ...
    print(f'Device: {device} - Inference Time: {round(t2 - t1, 5)}s')
    print(f'Version: {version} - Num parameters: {n_params}')
    print(f'Input shape: {x.shape}')
    print('Output shape:')
    for i, feature in enumerate(features, 1):
        print(f'\tx{i} - shape: {feature.shape}')
